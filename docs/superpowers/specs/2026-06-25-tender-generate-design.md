...
新增 `tender-generate` 阶段（CLI：`tender-insights generate`），在招标文件**已解读**的前提下，生成**投标响应目录方案**（非章节正文）。
...
| Viewer 关联 | **A** — 绑定解读会话；`/interpret` 加「生成目录」；新页 `/generate?session_id=...` |
...
| `generate/session.json` | 按步/一次性运行的状态机 |
| `llm_calls.jsonl` | 与 interpret 共用，追加 `generate_*` 记录 |
| `manifest.json` | 新增 `stages.generate` |
...
| 路径 | 职责 |
|------|------|
| `tender_insights/generate/models.py` | Pydantic 模型 |
| `tender_insights/generate/prompts.py` | `GENERATE_SYSTEM`、`REFINE_SYSTEM`（固定前置） |
| `tender_insights/generate/context.py` | 组装解读/brief/模板/废标/评分上下文 |
| `tender_insights/generate/excerpt.py` | 按节点从 `interpret/source_content.md` 选取摘录（≤2000 字；<200 字与后续段落拼接） |
| `tender_insights/generate/extractor.py` | 主流程：`generate_workspace()` |
| `tender_insights/generate/session.py` | 读/写 `generate/session.json`，步进逻辑 |
| `tender_insights/generate/accept.py` | draft → `bid_outline.json` + `bid_outline.md` |
| `tender_insights/generate/render.py` | draft/outline → Markdown |
| `tender_insights/api.py` | 暴露 `generate_bid_outline()`、`accept_bid_outline()`、`continue_generate()` |
| `tender_insights/cli/main.py` | `generate` 子命令 |
| `viewer/viewer/routes/generate.py` | REST API |
| `viewer/viewer/services/generate_pipeline.py` | 后台任务 + 进度 |
| `viewer/viewer/static/generate.html` + `generate.js` | 测试页 UI |
...
load interpretation.json + tender_brief.json? + templates/index.json?
    ↓
Step 0: generate_initial (LLM)
    → 输入：固定 GENERATE_SYSTEM + 解读概要 + brief + directory_requirements
           + 废标/评分摘要 + 模板清单
...
Step 1…N: for each node in preorder(node_queue):
    generate_node (LLM)
    → 输入：固定 REFINE_SYSTEM + 当前完整目录树 + 目标节点
...
|------|-----------|---------------------|--------------|
| 初始生成 | `generate_initial` | `GENERATE_SYSTEM`：角色、输出 JSON schema、废标/评分对齐规则、撰写规范要求 | 解读 overview、brief、tender_brief 五字段、directory_requirements 树、废标/评分明细摘要、模板清单 |
| 节点完善 | `generate_node` | `REFINE_SYSTEM`：在保持整体结构前提下完善单节点、返回**完整树**、引用 id 规则 | 当前完整目录 JSON、目标 `node_id`、节点相关摘录、关联废标/评分全文 |
...
- 启动 generate 前设置 `INTERPRET_LOG_JSONL={workspace}/llm_calls.jsonl`
...
on_progress("generate", {
    ...
    "current": 3,
    "total": 14,
    "step": "generate_node",       # generate_initial | generate_node | awaiting_accept
    "node_id": "bid-003",
    "node_title": "技术方案",
})
```
...
```bash
# 一次性生成（默认）
tender-insights generate ./output/my-bid

# 按步：仅初始目录
tender-insights generate ./output/my-bid --step --once

# 按步：继续下一节点
tender-insights generate ./output/my-bid --continue

# 确认落盘
tender-insights generate ./output/my-bid --accept

# 从头重来
tender-insights generate ./output/my-bid --restart --step --once
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/generate/sessions/{session_id}/start` | `?mode=auto\|step` 启动 |
| `POST` | `/generate/sessions/{session_id}/continue` | 按步继续 |
| `POST` | `/generate/sessions/{session_id}/accept` | 确认落盘 |
| `GET` | `/generate/jobs/{job_id}` | 进度轮询 |
| `GET` | `/generate/sessions/{session_id}/draft` | 当前 draft 树 |
| `GET` | `/generate/sessions/{session_id}/llm-calls` | 复用 llm_calls 读取逻辑 |
...
扩展 `InterpretJobState` 或新增 `GenerateJobState`（`job_kind="generate"`），字段与 interpret job 对齐：`progress_percent`、`step_current`、`step_total`、`message`、`detail`。
...
失败时 `generate/session.json` 保留 `status=failed` + `error`，允许 `--restart`。

---

## 10. 测试计划

### 10.1 单元测试（`tests/tender_insights/unit/test_generate.py`）
...
- `viewer/tests/api/test_generate_api.py`：start / continue / accept / draft
- `viewer/tests/unit/test_generate_pipeline.py`：进度回调字段
- 静态资源：`generate.html` 含进度条与继续按钮
...
- 页面上手工编辑节点后保存
- Word / docx 导出
- 自动串联进 `tender-insights all`（可后续加 `--with-generate`）
- 新建独立 Cursor skill 文件（实现完成后补 `tender-generate/SKILL.md`）

---

## 12. 实现顺序建议

1. `tender_insights/generate/*` 模型 + 初始生成 + 单节点 refine + session
2. CLI `generate` 子命令 + 契约测试
3. `accept` + `render` + Markdown 导出
4. Viewer API + pipeline + 进度
5. `generate.html` / `generate.js` + interpret 页入口
