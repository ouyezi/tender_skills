...
| E | `template_plan`, `template_extract`, `legal_section_review` | template + legal（已完成） |
...
- [x] 15 个 call_type 均有 local handler（OCR 走 ocr_client 分发）。
...
- [x] Batch E：`template_plan` / `template_extract` / `legal_section_review` local handler + 业务接线完成。
- [x] `grep` 业务包内无面向这 15 类的裸 `llm_client.complete` / `extract_json_model`（仅 handlers 与 deprecated `llm_extractor` 残留）。
- [x] `AGENT_INVOKE_MODE=local` 默认相关单元测试绿。
- [x] `invoke_json_model` / `extract_json_via_agent` 重试不注入 validation feedback。
- [x] 日志 `call_type` 与 `scripts/agents/*/enName` 对齐。
